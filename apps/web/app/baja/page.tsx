...
function BajaContent() {
  ...
  // processed=true immediately when no API call needed; false while waiting for fetch
  const needsFetch = !status && !!email
  const [processed, setProcessed] = useState(!needsFetch)
  const [unsubOk, setUnsubOk] = useState(false)

  // Only call API when arriving via direct link from email (no status param yet)
  useEffect(() => {
    if (!needsFetch) return
    fetch(`/api/unsubscribe?e=${encodeURIComponent(email!)}`)
      .then((res) => { setUnsubOk(res.ok) })
      .catch(() => { setUnsubOk(false) })
      .finally(() => setProcessed(true))
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])

  const isOk = status === "ok" || (processed && unsubOk)
  const isError = status === "error" || (processed && !status && !unsubOk && !!email)
          ...
          <div role="alert">
            ...
            <FadeIn delay={0.1}>
              <p className="text-base text-[var(--c-text-muted)] leading-relaxed mb-10 max-w-md mx-auto">
                ...
                <a href="mailto:contacto@cerebrosesponjosos.com" className="text-[var(--c-text)] underline">
          ...
          fallback={
            <main className="relative z-10 px-6 pt-32 pb-32">
              <div className="max-w-2xl mx-auto text-center" role="status" aria-live="polite">
                <p className="text-[var(--c-text-muted)]">Cargando...</p>
              </div>
            </main>
          }
        >
