    ...
    <div className="overflow-hidden rounded-2xl border border-[var(--c-border)] bg-[var(--c-surface)]">
      <div className="flex items-center justify-between border-b border-[var(--c-border)] px-5 py-3.5">
        <p className="text-[14px] font-semibold text-[var(--c-text)]">
          ...
          <span className="font-normal text-[var(--c-text-muted)]">esta semana</span>
        </p>
        <Link
          href={actionHref}
            ...
            return (
              <li key={item.id} className={cn(i > 0 && "border-t border-[var(--c-border)]")}>
                <a
                  ...
                  rel="noopener"
                  className="group grid grid-cols-[56px_1fr_auto] items-center gap-4 px-5 py-4 transition-colors hover:bg-[var(--c-surface-2)]"
                >
                  {/* Thumbnail — always show platform color if no image */}
                  <div className="relative h-14 w-14 shrink-0 overflow-hidden rounded-[10px]">
                    {item.thumbnail ? (
                      ...
                      <div className={cn("flex h-full w-full items-center justify-center text-white", cfg.iconBg)}>
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-[22px] font-bold leading-none tracking-tight text-[var(--c-text)]">
                      {item.metricValue}
                    </p>
                    {item.metricLabel && (
                      <p className="mt-0.5 text-[10px] uppercase tracking-wider text-[var(--c-text-muted)]">
                        {item.metricLabel}
                      </p>
                    )}
                  </div>
